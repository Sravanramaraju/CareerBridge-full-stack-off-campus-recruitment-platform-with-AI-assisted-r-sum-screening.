...
}).refine((values) => values.experienceMax >= values.experienceMin, { message: 'Maximum must be greater than minimum.', path: ['experienceMax'] }).refine((values) => values.hideSalary || values.salaryMax >= values.salaryMin, { message: 'Maximum must be greater than minimum.', path: ['salaryMax'] });
        ...
        {step === 2 && (
          <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
            <section className="surface-card p-6 sm:p-8"><h2 className="font-heading text-xl font-bold">Application and review</h2><p className="mt-1 text-xs text-[var(--cb-text-muted)]">Set a realistic deadline and review the candidate-facing summary.</p><div className="mt-6 grid gap-5">
              ...
              <FormField label="Screening questions" helper="Optional. Add one question per line, up to five.">{(field) => <TextArea {...field} className="min-h-28" placeholder="Are you available to work in the listed location?" {...register('screeningQuestions')} />}</FormField>
            </div></section>
